...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ faster, setFaster ] = useState(localStorage.getItem('orderFasterParams') === 'true');
  const [ time, setTime ] = useState(localStorage.getItem('orderTimeParams'));
  const [ selfService, setSelfService ] = useState(localStorage.getItem('orderSelfServiceParams') === 'true');
  const area = foodAreas.filter(area => area.id === areaId)[0];
  const item = area.items.filter(item => item.id === itemId)[0];

  useEffect(() => {
    localStorage.setItem('orderTimeParams', time);
    localStorage.setItem('orderFasterParams', faster.toString());
    localStorage.setItem('orderSelfServiceParams', selfService.toString());
